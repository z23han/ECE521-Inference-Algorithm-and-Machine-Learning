...
sys.stdout = Logger(task_dir + '/task5'+cur_time+'.log')
...
def model(tf_val, l1_weights, l1_biases, l2_weights, l2_biases, prob):
    logits1 = tf.matmul(tf_val/255., l1_weights) + l1_biases
    hidden = tf.nn.relu(logits1)
    hidden_drop = tf.nn.dropout(hidden, prob)
    return tf.matmul(hidden_drop, l2_weights) + l2_biases


### get the model log-likelihood
def modelEval(l1_weights, l1_biases, l2_weights, l2_biases, tf_dataset, tf_labels, prob):
    logits = model(tf_dataset, l1_weights, l1_biases, l2_weights, l2_biases, prob)
...
# at run time with a training minibatch.
    ...
    train_logits, train_pred, train_loss, train_log = modelEval(layer1_weights, layer1_biases, layer2_weights, layer2_biases, x, y, keep_prob)
    ...
    valid_logits, valid_pred, valid_loss, valid_log = modelEval(layer1_weights, layer1_biases, layer2_weights, layer2_biases, tf_valid_dataset, tf_valid_labels, keep_prob)
    test_logits, test_pred, test_loss, test_log = modelEval(layer1_weights, layer1_biases, layer2_weights, layer2_biases, tf_test_dataset, tf_test_labels, keep_prob)
