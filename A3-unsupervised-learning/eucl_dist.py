...
def euclidean_dist(mat1, mat2):
    ...
    tf_mat1 = tf.convert_to_tensor(mat1, dtype=tf.float64)
    tf_mat2 = tf.convert_to_tensor(mat2, dtype=tf.float64)
    tf_rep_mat1 = tf.reshape(tf.tile(tf_mat1, [row2,1]), [row2, row1, col1])
    tf_rep_mat2 = tf.reshape(tf.tile(tf_mat2, [1,row1]), [row2, row1, col2])
    sum_squares = tf.reduce_sum(tf.square(tf.sub(tf_rep_mat1, tf_rep_mat2)), [2])
    return sum_squares
